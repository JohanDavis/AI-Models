...
# Feature scaling: divide by max values of each feature
x_max = np.max(x, axis=0)  # Get max value for each column/feature
x_scaled = x / x_max
...
w = np.zeros(x_scaled.shape[1]) # [0, 0, 0]
b = 0
...
while True:
    ...
    newW = new_w(w, b, alpha, x_scaled, y)
    newB = new_b(w, b, alpha, x_scaled, y)
...
while True:
    ...
    features_scaled = features / x_max  # Scale input features
    predicted = np.dot(w, features_scaled) + b
